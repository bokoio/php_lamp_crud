...
/*FN Verificar as colunas de todas as function, estao erradas.*/
var page = 1;
var current_page = 1;
var total_page = 0;
var is_ajax_fire = 0;
...
function manageData() {
    $.ajax({
    	...
    	total_page = Math.ceil(data.total/20);
    	current_page = page;
...
$(".crud-submit").click(function(e){
    ...
    if(title != '' && description != ''){ /*Alterar essa validação*/
        $.ajax({
            dataType: 'json',
            type:'POST',
            url: url + form_action,
            ...
            getPageData();
            $(".modal").modal('hide');
            toastr.success('Curso cadastrado com sucesso.', 'Success Alert', {timeOut: 5000});
        });
    }else{
        alert('You are missing title or description.')/*ALterar esse alerta.*/
    }
...
$("body").on("click",".remove-item",function(){
    ...
    $.ajax({
        ...
        c_obj.remove();
        toastr.success('Curos removido com sucesso.', 'Success Alert', {timeOut: 5000});
        getPageData();
    });
...
$(".crud-submit-edit").click(function(e){
    ...
    if(title != '' && description != ''){
        $.ajax({
            ...
            getPageData();
            $(".modal").modal('hide');
            toastr.success('Curso atualizado com sucesso.', 'Success Alert', {timeOut: 5000});
        });
    }else{
